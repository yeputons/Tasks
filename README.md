...
**Дедлайн**: начало лекции 1 октября.
...
То:
1. Скачайте [отсюда](ftp://ftp.graphicsmagick.org/pub/GraphicsMagick/windows/) файл ```GraphicsMagick-1.3.28-Q8-win64-dll.exe```
2. Установите и не забудьте перезапустить среду разработки (чтобы при выполнении программы ```PATH``` был обновленный при установке GraphicsMagick)
...
Максимум нужно реализовать суммирование бинарным деревом внутри рабочей группы и с минимальным числом барьеров за счет ```#if (WARP_SIZE > N)``. Еще потенциальные оптимизации - ручной unrolling циклов, обработка каждой рабочей группы больше чем WorkGroupSize элементов и т.п..
